...
export async function mkdir(name: string, parentId?: string | null): Promise<FileItem> {
...
export async function moveFile(id: string, parentId: string | null): Promise<void> {
    await client.put(`/files/${id}/move`, { parent_id: parentId })
}

export async function copyFile(fileId: string, parentId?: string | null): Promise<FileItem> {
...
export async function getFileURL(id: string): Promise<{ url: string; markdown: string }> {
